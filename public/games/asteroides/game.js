...
let powerUpSpawned;
let killsSinceSpawn;
let paused = false;   // controlado por el contenedor (Player) y por la tecla P
...
const lastEmit = { score: null, lives: null, level: null, over: false, triple: false };
...
function emitState() {
  ...
  const tripleNow = ship.tripleShot > 0;
  if ( tripleNow !== lastEmit.triple ) {
    lastEmit.triple = tripleNow;
    postToHost( { type: 'powerup', name: 'triple', remaining: tripleNow ? ship.tripleShot : 0 } );
  }

  const overNow = state === 'gameover';
...
function loop( ts ) {
  ...
  // Tecla P: alterna pausa (funciona también estando en pausa).
  if ( pressed( 'KeyP' ) ) setPaused( !paused );

  if ( !paused ) update( dt );
  draw();
  if ( paused ) drawOverlay( 'EN PAUSA', 'P O EL BOTÓN PARA CONTINUAR' );
