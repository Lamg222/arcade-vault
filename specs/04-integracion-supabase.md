...
- Variables de entorno públicas: `NEXT_PUBLIC_SUPABASE_URL` (URL del proyecto) y `NEXT_PUBLIC_SUPABASE_ANON_KEY` (clave anónima pública — segura en navegador porque RLS la limita). El usuario provee los valores; van en `.env.local`; plantilla en `.env.example`.
...
- **Env vars** (ambas públicas, prefijo `NEXT_PUBLIC_` — expuestas al bundle de navegador): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
...
- **REQ-02** — The system MUST leer `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` desde el entorno (`process.env`).
...
- **REQ-08** — The system MUST verificar conectividad real contra el proyecto Supabase (una llamada `auth.getUser()` que retorna sin error de red/config, con usuario `null` al no haber sesión).
...
2. **Cliente browser** `app/lib/supabase/client.ts`: `createClient()` con `createBrowserClient(url, anonKey)`, validando ambas vars (REQ-05). → commit.
3. **Cliente server** `app/lib/supabase/server.ts`: `async createClient()` con `createServerClient(url, anonKey, { cookies: { getAll, setAll } })`, `setAll` en `try/catch` (REQ-07), validando vars (REQ-05). → commit.
4. **Verificación de conexión** (REQ-08): Route Handler temporal `app/api/supabase-check/route.ts` que instancia el cliente server, llama `auth.getUser()`, retorna `{ ok, user }`. Probar en navegador/curl. Confirmada la conexión, evaluar borrar el handler (ver D-05). → commit final = acceptance.
...
- **AC-03 (conexión real, REQ-08)** — Given el proyecto Supabase provisionado, When hago GET a `/api/supabase-check`, Then respuesta `200` con `{ ok: true, user: null }` (confirma red + URL + anon key correctas).
- **AC-04 (edge: var faltante, REQ-05)** — Given `.env.local` sin `NEXT_PUBLIC_SUPABASE_ANON_KEY`, When creo cualquier cliente, Then lanza error nombrando la variable ausente (no `undefined` silencioso).
...
- **AC-06 (edge: URL malformada)** — Given `NEXT_PUBLIC_SUPABASE_URL` con valor inválido, When llamo `auth.getUser()` vía `/api/supabase-check`, Then respuesta con `ok: false` y error de red, no crash del servidor.
...
3. `npm run dev` + `curl localhost:3000/api/supabase-check` → `{ ok: true, user: null }` (AC-03, prueba de red real).
4. Borrar temporalmente `NEXT_PUBLIC_SUPABASE_ANON_KEY` de `.env.local` → recarga → el error nombra la var (AC-04). Restaurar.
